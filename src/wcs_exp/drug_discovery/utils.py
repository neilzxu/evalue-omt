...
def BH(pvals, q=0.1):
    ...
    id_in = [
        j for j in range(mh) if df_sorted.iloc[j, 1] <= df_sorted.iloc[j, 2]
    ]
    if len(id_in) == 0:
        return (np.array([]))
    else:
...
def weighted_BH(calib_scores, calib_weights, test_scores, test_weights, q=0.1):
    ...
    df_all = pd.concat((pd.DataFrame({
        "score": calib_scores,
        "weight": calib_weights,
        "cal": 1
    }), pd.DataFrame({
        "score": test_scores,
        "weight": test_weights,
    ...
    for j in range(all_sorted.shape[0]):
        if all_sorted[j, 2] == 0:
            p_vals.append(
                (np.sum(all_sorted[range(j), 1] * all_sorted[range(j), 2]) +
                 all_sorted[j, 1] * np.random.uniform(size=1)[0]) /
                (sum_calib_weight + all_sorted[j, 1]))
        else:
            p_vals.append(-1)
    ...
    idx_smaller = [
        j for j in range(ntest)
        if df_test_sorted.iloc[j, 3] <= df_test_sorted.iloc[j, 4]
    ]

    if len(idx_smaller) == 0:
        return (np.array([]))
    else:
        idx_sel = np.array(df_test_sorted.index[range(np.max(idx_smaller) +
                                                      1)])
        return (idx_sel)
...
def weighted_CS(calib_scores, calib_weights, test_scores, test_weights, q=0.1):
    ...
    for j in range(ntest):
        ...
        w_pvals[j] = (np.sum(calib_weights[calib_scores < test_scores[j]]) +
                      (np.sum(calib_weights[calib_scores == test_scores[j]]) +
                       test_weights[j]) * np.random.uniform(size=1)[0]) / (
        ...
        idx_small_j = [
            s for s in range(ntest) if df_j.iloc[s, 1] <= df_j.iloc[s, 2]
        ]
    ...
    if len(pj_sel0) == 0:
        ...
        df_sel0 = df_all[df_all['pval'] <= df_all['c']].sort_values(
            by='hete_Rj')
        df_sel0['threshold'] = np.linspace(1,
                                           df_sel0.shape[0],
                                           num=df_sel0.shape[0])
        smaller = [
            j for j in range(df_sel0.shape[0])
            if df_sel0.iloc[j, 3] <= df_sel0.iloc[j, 6]
        ]
        if len(smaller) == 0:
        ...
        smaller = [
            j for j in range(df_sel0.shape[0])
            if df_sel0.iloc[j, 4] <= df_sel0.iloc[j, 6]
        ]
        if len(smaller) == 0:
        ...
        smaller = [
            j for j in range(df_sel0.shape[0])
            if df_sel0.iloc[j, 5] <= df_sel0.iloc[j, 6]
        ]
        if len(smaller) == 0:
        ...
        return np.array(
            df_sel0['id']), idx_sel_hete, idx_sel_homo, idx_sel_dete  #, Cj
